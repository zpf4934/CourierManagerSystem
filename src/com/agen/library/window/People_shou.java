...
public class People_shou extends JFrame implements ActionListener {
		...
		public People_shou() {
			...
			lblBookOrAuthor = new JLabel("姓    名：");
			...
			if (e.getSource() == btnSearch) {
				...
				if (name != null && !name.equals("")){
				    sql = "select receivephone,receivename,"
				    		+ "receiveadress,id from receive where receivename like " + "'" + name + "'";
				}
				else if(starttime != null && !starttime.equals("") && 
						endtime!= null && !endtime.equals("")){
					sql = "select receivephone,receivename,"
				    		+ "receiveadress,id from receive where riqi BETWEEN " + "'" +starttime +"'" + " AND " + "'" +  endtime+ "'" ;
				}
			
				StuModel_shou sm = new StuModel_shou(sql);
				tabel.setModel(sm);
			}
