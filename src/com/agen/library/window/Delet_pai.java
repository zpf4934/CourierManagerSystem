...
public class Delet_pai extends JFrame implements ActionListener {
			...
			if(e.getSource()==btnExit){
				...
				if(e.getSource() == delet_all){
					...
					if(starttime != null && !starttime.equals("") && 
							endtime!= null && !endtime.equals("")){
						sql = "DELETE FROM receive where riqi BETWEEN " + "'" +starttime +"'" + " AND " + "'" +  endtime+ "'" ;
						StuModel sm = new StuModel(sql);
						tabel.setModel(sm);
					}else{
				...
				if(e.getSource() == deletone){
					...
					String name = this.txtSearch.getText().trim();
					if(name != null && !name.equals("")){
						sql = "DELETE FROM receive where id like " + name ;
						StuModel sm = new StuModel(sql);
						tabel.setModel(sm);
					}else{
						JOptionPane.showMessageDialog(null, "请认真输入！", "消息", 1);
