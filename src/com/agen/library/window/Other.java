...
public class Other  extends JFrame{
		...
		JTextField txtBookName,  txtISBN;
		JLabel lblBookName, lblAuthor, lblISBN;
		...
		JComboBox txtAuthor;
		String[] other = {"退件","问题滞留","问题返库"};
		public Other(JFrame jf, String s, boolean b,String id ) {
			// 创建组件并添加组件
			...
			lblBookName = new JLabel("*处理时间：");
			txtBookName = new JTextField(10);
			txtBookName.setText(DateUtil.getDateTime());
			lblAuthor = new JLabel("情况说明：");
			txtAuthor = new JComboBox(other);

			lblISBN = new JLabel("备   注：");
			txtISBN = new JTextField(10);
			btnAdd = new JButton("确定");// 按钮创建
		...
		public void addactionPerformed(ActionEvent e) {
			...
			if (textjp6.getText() != null && !textjp6.getText().equals("")
				...
				int option = JOptionPane.showConfirmDialog(this, "你确定要处理么？", "提示",
					...
					try {
						...
						ct = DriverManager
								.getConnection(
										...
										"sa", "123456");
						// 编译语句
						String sql = "UPDATE receive SET signtime=?,signs=?,beizhu=? WHERE id=?";
						ps = ct.prepareStatement(sql);
						// 给参数赋值
						ps.setString(1, txtBookName.getText());
						ps.setString(2, other[txtAuthor.getSelectedIndex()]);
						if (txtISBN.getText() != null && !txtISBN.getText().equals("")){
						ps.setString(3, txtISBN.getText());
						}
						else{
							ps.setString(3, "");
						}
						ps.setString(4, textjp6.getText().trim());
						// 执行操作
						ps.executeUpdate();
					} catch (Exception e1) {
						e1.printStackTrace();
